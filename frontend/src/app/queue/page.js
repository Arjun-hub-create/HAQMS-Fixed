...
export default function QueueMonitor() {
  ...
  // FIX: Wrapped in useCallback to keep stable reference for the useEffect dependency
  const fetchQueueData = useCallback(async () => {
    try {
      ...
      // FIX: Handle both old array format and new { success, data } shape
  ...
  useEffect(() => {
    ...
    /**
     * FIX (Memory Leak - Critical): The original code started a setInterval but
     * never returned a cleanup function. Every mount of this page created a new
     * polling interval that ran forever — causing:
     *   - Memory bloat from accumulating intervals
     *   - setState calls on unmounted components (React warning/crash)
     *   - Unnecessary server load from duplicate polls
     *
     * Fix: Return () => clearInterval(intervalId) to clean up on unmount.
     */
    ...
    // FIX: Cleanup — cancel the interval when the component unmounts
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 sm:p-8">
        ...
        {error && (
          <div className="p-4 mb-6 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-500 flex items-center gap-3 text-sm">
            ...
            <div>
              <strong>Sync Error:</strong> {error} — Please verify the backend API server is online.
            </div>
          </div>
        )}
        ...
        {loading && tokens.length === 0 ? (
          ...
          <div className="glass p-12 text-center rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
            ...
            <p className="mt-2 text-slate-500 dark:text-slate-400 text-sm max-w-md mx-auto">
              There are currently no patient check-ins registered for today. Use the receptionist
              portal in the Staff Dashboard to check-in patients.
            </p>
          </div>
        ) : (
