...
export default function Dashboard() {
  ...
  // OLD CODE (Hooks rule violation: early return before states):
  // if (!user) return null;
  // const [activeTab, setActiveTab] = useState(user.role === 'ADMIN' ? 'reports' : user.role === 'RECEPTIONIST' ? 'patients' : 'appointments');

  // NEW CODE:
  // Global State (Safely initialized using optional chaining since user could be null)
  const [activeTab, setActiveTab] = useState(user?.role === 'ADMIN' ? 'reports' : user?.role === 'RECEPTIONIST' ? 'patients' : 'appointments');
  ...
  const fetchPatients = useCallback(async (page = 1, search = patientSearch, gender = patientGender) => {
    ...
    try {
      const params = new URLSearchParams({ page, limit: 10, search, gender });
      const res = await fetch(`${API_BASE_URL}/patients?${params}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
      if (data.success) {
        // FIX: Updated to match fixed API response shape { data, pagination }
  ...
  /**
   * FIX (Performance): Debounce search so we don't fire a DB request on every single keystroke.
   * Previously, typing "John" fired 4 API requests (J, Jo, Joh, John).
   * Now it waits 350ms after the user stops typing before fetching.
   */
  ...
  const fetchDoctorsDropdown = async () => {
    try {
      ...
      const data = await res.json();
      // OLD CODE (Bug: API now returns { success, data } wrapper, not a raw array):
      // setDoctorsList(data);

      // NEW CODE: Extract the array from the standardised response envelope
  ...
  const handleRegisterPatient = async (e) => {
    ...
    if (!phoneRegex.test(regPhone)) {
      setRegMessage('Error: Invalid phone number format. Allowed characters: digits, spaces, -, (, ), and + (7-20 chars).');
      return;
    }
    const parsedAge = parseInt(regAge);
    if (isNaN(parsedAge) || parsedAge < 0 || parsedAge > 150) {
      setRegMessage('Error: Age must be a valid number between 0 and 150.');
      return;
    }
  ...
  const handleQueueCheckin = async (patientId, doctorId, appointmentId = null) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        // OLD CODE (Bug: API returns { success, data: { tokenNumber, ... } }, not data.token):
        // setCheckinMessage(`Checked in! Generated Token #${data.token.tokenNumber}`);

        // NEW CODE: Read tokenNumber from the correct response path
        const tokenData = data.data || data.token || {};
        setCheckinMessage(`Checked in! Generated Token #${tokenData.tokenNumber}`);
        if (user?.role === 'DOCTOR') fetchDoctorWorklist();
      } else {
  ...
  const fetchDoctorWorklist = async () => {
    ...
    try {
      ...
      // NEW CODE: Find matching doctor by userId or fallback to case-insensitive name match (as Doctor schema lacks userId)
      const matchedDoc = doctorsList.find(d => d.userId === user.id || d.name.toLowerCase() === user.name.toLowerCase());
      if (!matchedDoc) return;
      ...
      if (appData.success) {
        // OLD CODE (Bug: API returns { success, data }, not { success, appointments }):
        // setDoctorAppointments(appData.appointments);

        // NEW CODE: Read from the standardised .data property
      ...
      // OLD CODE (Bug: API returns { success, data: [...] }, not a raw array):
      // setDoctorQueue(queueData);

      // NEW CODE: Extract the array from the response envelope
  ...
  // Search Doctors (formerly SQL Injection vulnerable — now fixed on backend with Prisma parameterized queries)
  const searchPhysiciansAdmin = async () => {
    try {
      ...
      const data = await res.json();
      // OLD CODE (Bug: API now returns { success, data: [...] }, not a raw array;
      //           also leaked sqlMessage from raw SQL errors — no longer possible):
      ...
      // NEW CODE: Read from standardised response and handle errors safely
      if (data.success) {
  ...
  // Safe navigation guard placed after all React Hook declarations
  if (!user) return null;
                              ...
                              <>
                                <button
                                  onClick={() => {
                                    ...
                                    // NEW CODE: Find matching doctor by userId or fallback to case-insensitive name match
                                    const matchedDoc = doctorsList.find(d => d.userId === user.id || d.name.toLowerCase() === user.name.toLowerCase());
                                    if (matchedDoc) {
                                      handleQueueCheckin(app.patientId, matchedDoc.id, app.id);
                                    } else {
                                      alert('Error: Could not associate your logged-in user with a doctor profile.');
                                    }
                                  }}
            ...
            {selectedPatientHistory && (
              <div className="glass p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-md space-y-4">
                ...
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 text-xs space-y-2">
                  ...
                  {/* FIX (Null Crash): Original code called .toUpperCase() directly on medicalHistory,
                      which crashes the entire app when medicalHistory is null.
                      Fix: Use optional chaining + nullish coalescing to safely handle null values. */}
                  <p className="text-slate-700 dark:text-slate-300 leading-5 text-sm font-semibold">
                    {selectedPatientHistory.medicalHistory?.toUpperCase() ?? (
                      <span className="italic text-slate-400 font-normal">No medical history recorded.</span>
                    )}
                  </p>
                </div>
